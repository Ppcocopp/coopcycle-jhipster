<template>
  <div>
    <h2 id="page-heading" data-cy="CustomerHeading">
      <span v-text="$t('blogApp.customer.home.title')" id="customer-heading">Customers</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('blogApp.customer.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CustomerCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('blogApp.customer.home.createLabel')"> Create a new Customer </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && customers && customers.length === 0">
      <span v-text="$t('blogApp.customer.home.notFound')">No customers found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="customers">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('blogApp.customer.adress')">Adress</span></th>
            <th scope="row"><span v-text="$t('blogApp.customer.balance')">Balance</span></th>
            <th scope="row"><span v-text="$t('blogApp.customer.phone')">Phone</span></th>
            <th scope="row"><span v-text="$t('blogApp.customer.mail')">Mail</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="customer in customers" :key="customer.id" data-cy="entityTable">
            ...
            <td>
              {{ customer.mail ? customer.mail.id : '' }}
            </td>
            <td class="text-right">
              <div class="btn-group">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="blogApp.customer.delete.question" data-cy="customerDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-customer-heading" v-text="$t('blogApp.customer.delete.question', { id: removeId })">
